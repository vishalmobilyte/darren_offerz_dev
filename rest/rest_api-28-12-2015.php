...
class Offerz_web_services
{
	...
	if( $this->params->email )
		...
		$sql="SELECT SQL_CALC_FOUND_ROWS I.*, T.name FROM invites I  JOIN teams T on I.team_id=T.id WHERE I.email='$email' AND I.is_accepted=0 $limit_condition";
	...
	if( $this->params->user_id )
		...
		WHERE UO.user_id = $user_id ORDER BY UO.created_at DESC $limit_condition";
	...
	if( $this->params->email )
		...
		$sql="SELECT I.*, T.*, C.screen_name FROM invites I JOIN teams T ON I.team_id = T.id JOIN clients C ON T.client_id=C.id
						WHERE I.email='$email' AND I.is_accepted=1";
		//echo $sql; die;
		$result=mysqli_query($conn,$sql);
		//$row=mysqli_fetch_array($result,MYSQLI_ASSOC);
