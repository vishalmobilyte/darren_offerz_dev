...
class Offerz_web_services
{
	...
	if( $this->params->email )
		...
		$sql="SELECT SQL_CALC_FOUND_ROWS I.*, T.name as name, CL.name as sponsor_name FROM invites I  JOIN teams T on I.team_id=T.id LEFT JOIN clients CL ON T.client_id = CL.id WHERE I.email='$email' AND I.is_accepted=0 $limit_condition";
	...
	if( $this->params->email && $this->params->team_id && $this->params->is_accepted )
		...
		if($is_accepted == '2'){
		$sql="DELETE FROM invites WHERE email='$email' AND team_id='$team_id'";
		}
		else{
	...
	// ================ GET ALL TEAMS JOINED BY THE MOBILE USERS ================
	...
	if( $this->params->email )
		...
		$sql="SELECT DISTINCT  I.*, T.*, C.screen_name, C.name as sponsor_name FROM invites I JOIN teams T ON I.team_id = T.id JOIN clients C ON T.client_id=C.id
						WHERE I.email='$email' AND I.is_accepted=1 GROUP BY sponsor_name";
		//echo $sql; die;
		$result=mysqli_query($conn,$sql);
		//$row=mysqli_fetch_array($result,MYSQLI_ASSOC);
